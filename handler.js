...
export async function handler(conn, rawMsg) {
  ...
  // ✅ حماية من يوزر spoofing — لو الـ sender بييجي برقم البوت والرسالة مش fromMe
  if (m && !rawMsg.key?.fromMe) {
    const botNum = (conn.user?.jid || '').replace(/@.+/, '').replace(/\D/g, '')
    const senderNum = (m.sender || '').replace(/@.+/, '').replace(/\D/g, '')
    if (botNum && senderNum === botNum) {
      console.log('[SECURITY] spoofing detected, ignoring message from:', m.sender)
      return
    }
  }
...
title: '', lastclaim: 0, lastweekly: 0
  ...
  // ✅ تأكد إن الـ sender مش رقم البوت نفسه (منع يوزر spoofing)
  const botJidNum = numOf(conn.user?.jid || '')
  const senderNum2 = numOf(m.sender)
  if (botJidNum && senderNum2 === botJidNum && !m.fromMe) {
    console.log('[SECURITY] blocked sender spoofing from:', m.sender)
    return
  }
  ...
  const isRealOwner = isOwnerJid(m.sender)
