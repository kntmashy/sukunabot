...
import * as _baileysAll from 'angularsockets';
import _baileys from 'angularsockets';
const _makeWaSocket = _baileys.default || _baileys;
const {
    proto,
    areJidsSameUser,
    ...
    let sock = Object.defineProperties(conn, {
        ...
        getJid: {
            value(sender) {
                ...
                // 1. جرب من conn.contacts أولاً (أسرع وأدق)
                const contacts = conn.contacts || {}
                for (const [cJid, c] of Object.entries(contacts)) {
                    if ((c?.lid === s || c?.id === s) && cJid.endsWith('@s.whatsapp.net')) {
                        conn.isLid[s] = String(cJid).decodeJid()
                        return conn.isLid[s]
                    }
                }

                // 2. جرب من group participants
    ...
    return Object.defineProperties(proto.WebMessageInfo.prototype, {
        ...
        sender: {
            get() {
                ...
                // 2. لو LID، جرب تحله من الرسالة نفسها أو contacts قبل الـ fallback
                const first = candidates[0] || ''
                if (first.endsWith('@lid')) {
                    // جرب من contextInfo في الرسالة
                    const msgKey = Object.keys(this.message || {})[0]
                    const ctxParticipant = this.message?.[msgKey]?.contextInfo?.participant
                    if (ctxParticipant && ctxParticipant.endsWith('@s.whatsapp.net')) {
                        return this.conn.getJid(ctxParticipant)
                    }
                    // جرب من conn.contacts
                    const contacts = this.conn?.contacts || {}
                    for (const [cJid, c] of Object.entries(contacts)) {
                        if ((c?.lid === first || c?.id === first) && cJid.endsWith('@s.whatsapp.net')) {
                            return this.conn.getJid(cJid)
                        }
                    }
                }
                // 3. جرب من group participants في الـ chat الحالي
                if (first.endsWith('@lid')) {
                    const chat = this.chat || this.key?.remoteJid || ''
                    const chatData = this.conn?.chats?.[chat]
                    const parts = chatData?.metadata?.participants || []
                    for (const p of parts) {
                        const pLid = p?.lid || p?.id || ''
                        if (pLid === first || String(pLid).decodeJid?.() === first) {
                            const resolved = p?.jid || p?.id || p?.phoneNumber
                            if (resolved && !String(resolved).endsWith('@lid')) {
                                return this.conn.getJid(String(resolved).replace(/:[0-9]+@/, '@'))
                            }
                        }
                    }
                }

                return this.conn.getJid(first)
            },
            enumerable: true
        },
        fromMe: {
                ...
                return Object.defineProperties(JSON.parse(JSON.stringify(typeof q === 'string' ? {
                    ...
                    sender: {
                        get() {
                            ...
                            // لو مش lid رجّع مباشرة
                            if (!raw.endsWith('@lid')) return conns.getJid(raw)
                            // جرب تحله من conn.contacts
                            const contacts = conns.conn?.contacts || conns.contacts || {}
                            for (const [cJid, c] of Object.entries(contacts)) {
                                if ((c?.lid === raw || c?.id === raw) && cJid.endsWith('@s.whatsapp.net')) {
                                    return conns.getJid(cJid)
                                }
                            }
                            // جرب من group participants
                            for (const chat of Object.values(conns.conn?.chats || conns.chats || {})) {
                                const parts = chat.metadata?.participants || []
                                const match = parts.find(p => p?.lid === raw || p?.id === raw)
                                if (match?.jid && !match.jid.endsWith('@lid')) return conns.getJid(match.jid)
                            }
        ...
        text: {
            get() {
                ...
                // 🔱 GOJO BOT - دعم الأزرار والقوائم
                const btnResp = this.message?.buttonsResponseMessage?.selectedButtonId
                const listResp = this.message?.listResponseMessage?.singleSelectReply?.selectedRowId
                const tplResp = this.message?.templateButtonReplyMessage?.selectedId
                let intResp = ''
                try {
                    const raw = this.message?.interactiveResponseMessage?.nativeFlowResponseMessage?.paramsJson
                    if (raw) intResp = JSON.parse(raw)?.id || ''
                } catch {}
                return typeof this._text === 'string' ? this._text : '' || (typeof text === 'string' ? text : (
                    text?.selectedDisplayText ||
                    text?.hydratedTemplate?.hydratedContentText ||
                    text
                )) || btnResp || listResp || tplResp || intResp || ''
            },
            set(str) {
        ...
        mentionedJid: {
            get() {
                // دور في كل أنواع الرسائل الممكنة
                ...
                const raw = (
                    ...
                    msg?.conversation?.contextInfo?.mentionedJid ||
                    msg?.buttonsMessage?.contextInfo?.mentionedJid ||
                    msg?.listMessage?.contextInfo?.mentionedJid ||
                    []
                ).filter(Boolean)
                return raw.map(jid => {
                    if (!jid.endsWith('@lid')) return this.conn.getJid(jid)
                    const contacts = this.conn?.contacts || {}
                    for (const [cJid, c] of Object.entries(contacts)) {
                        if ((c?.lid === jid || c?.id === jid) && cJid.endsWith('@s.whatsapp.net')) {
                            return this.conn.getJid(cJid)
                        }
                    }
                    for (const chat of Object.values(this.conn?.chats || {})) {
                        const parts = chat.metadata?.participants || []
                        const match = parts.find(p => p?.lid === jid || p?.id === jid)
                        if (match?.jid && !match.jid.endsWith('@lid')) return this.conn.getJid(match.jid)
                    }
                    return this.conn.getJid(jid)
                })
            },
            enumerable: true
        },
        ...
        /**
         * 🔱 GOJO BOT - sendList (قائمة تفاعلية)
         */
        send: {
            /**
             * Send message to same chat (alias for sendMessage)
             * @param {String|Object} content - text or message object
             * @param {Object} options
             */
            async value(content, options = {}) {
                if (!this.conn || !this.chat) return
                const msg = typeof content === 'string' ? { text: content } : content
                return this.conn.sendMessage(this.chat, { ...msg, ...options }, { quoted: this, ...options })
            },
            enumerable: true
        },
        sendList: {
            async value(jid, title = '', text = '', footer = '', buttonText = '📋 اختر', sections = [], quoted, options = {}) {
                const listMessage = {
                    text: text || '',
                    footer: footer || '',
                    title: title || '',
                ...
                return await this.conn?.sendMessage(jid, listMessage, { quoted, ...options })
            },
            enumerable: true
        },
        /**
         * 🔱 GOJO BOT - sendInteractiveButton (أزرار نيتف فلو)
         */
        sendInteractiveButton: {
            async value(jid, body = '', footer = '', buttons = [], quoted, options = {}) {
                const nativeFlowButtons = buttons.map(btn => ({
                    name: btn.name || 'quick_reply',
                    buttonParamsJson: JSON.stringify({
                        display_text: btn.displayText || btn.text || '',
                        id: btn.buttonId || btn.id || btn.displayText || ''
                    })
                }))
                const message = {
                    interactiveMessage: {
                ...
                return await this.conn?.relayMessage(jid, message, { messageId: options.messageId, quoted, ...options })
            },
            enumerable: true
        }
    })
